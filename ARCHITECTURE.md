...
- `frontend/` — Vitest unit suite for pure logic (doc-type normalization/routing contract, affiliate selection, report formatting helpers). Run: `cd frontend && npm test`. Node environment, no DOM; `*.test.ts` files are excluded from the production `tsc -b` build.
